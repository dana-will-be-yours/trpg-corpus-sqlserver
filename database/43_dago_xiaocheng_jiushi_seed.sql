...
/*
    da_go 1.12.0 小城舊事劇本種子
    劇本時間：大興十年
    劇本地點：天津郡 常山縣
    研究者隱藏目標：在此地找到工作生活一年

    安全原則：本檔不使用 DELETE / TRUNCATE。
    執行正式資料庫前仍請先備份 TRPG_Corpus_DB。
*/
...
IF COL_LENGTH(N'dbo.DaGo_Authoring_Scenario_Location', N'is_active') IS NULL
BEGIN
    ALTER TABLE dbo.DaGo_Authoring_Scenario_Location
        ADD is_active BIT NOT NULL CONSTRAINT DF_DaGo_Authoring_Scenario_Location_is_active DEFAULT 1;
END;
GO

IF COL_LENGTH(N'dbo.DaGo_Authoring_Scenario_NPC', N'is_active') IS NULL
BEGIN
    ALTER TABLE dbo.DaGo_Authoring_Scenario_NPC
        ADD is_active BIT NOT NULL CONSTRAINT DF_DaGo_Authoring_Scenario_NPC_is_active DEFAULT 1;
END;
GO

IF COL_LENGTH(N'dbo.DaGo_Authoring_Scenario_Quest', N'is_active') IS NULL
BEGIN
    ALTER TABLE dbo.DaGo_Authoring_Scenario_Quest
        ADD is_active BIT NOT NULL CONSTRAINT DF_DaGo_Authoring_Scenario_Quest_is_active DEFAULT 1;
END;
GO
...
WHERE scenario_code = N'changshan_year';
GO

MERGE dbo.DaGo_Authoring_Scenario AS target
USING (VALUES
    (N'xiaocheng_jiushi', N'小城舊事', N'大興十年天津郡常山縣',
     ...
     N'assets/data/scenarios/xiaocheng-jiushi.json', 10)
...
(
    ...
    PRIMARY KEY (scenario_code, location_code)
);

INSERT INTO @LocationSeed VALUES
...
USING @LocationSeed AS source
ON target.scenario_code = source.scenario_code AND target.location_code = source.location_code
WHEN MATCHED THEN UPDATE SET
    ...
    is_active = 1
WHEN NOT MATCHED THEN INSERT
    (scenario_code, location_code, location_name, district_name, location_type, allowed_for_player, story_note, sort_order, is_active)
    VALUES
    (source.scenario_code, source.location_code, source.location_name, source.district_name, source.location_type, source.allowed_for_player, source.story_note, source.sort_order, 1);

UPDATE target
SET is_active = 0
FROM dbo.DaGo_Authoring_Scenario_Location AS target
WHERE target.scenario_code = N'xiaocheng_jiushi'
  AND NOT EXISTS (
      SELECT 1 FROM @LocationSeed AS source
      WHERE source.scenario_code = target.scenario_code
        AND source.location_code = target.location_code
  );
GO

DECLARE @NpcSeed TABLE
(
    scenario_code NVARCHAR(80) NOT NULL,
    ...
    sort_order INT NOT NULL,
    PRIMARY KEY (scenario_code, npc_code)
);

INSERT INTO @NpcSeed VALUES
...
USING @NpcSeed AS source
ON target.scenario_code = source.scenario_code AND target.npc_code = source.npc_code
WHEN MATCHED THEN UPDATE SET
    ...
    is_active = 1
WHEN NOT MATCHED THEN INSERT
    (scenario_code, npc_code, npc_name, npc_role, default_location_code, tag_json, gm_note, sort_order, is_active)
    VALUES
    (source.scenario_code, source.npc_code, source.npc_name, source.npc_role, source.default_location_code, source.tag_json, source.gm_note, source.sort_order, 1);

UPDATE target
SET is_active = 0
FROM dbo.DaGo_Authoring_Scenario_NPC AS target
WHERE target.scenario_code = N'xiaocheng_jiushi'
  AND NOT EXISTS (
      SELECT 1 FROM @NpcSeed AS source
      WHERE source.scenario_code = target.scenario_code
        AND source.npc_code = target.npc_code
  );
GO

DECLARE @QuestSeed TABLE
(
    scenario_code NVARCHAR(80) NOT NULL,
    ...
    sort_order INT NOT NULL,
    PRIMARY KEY (scenario_code, quest_code)
);

INSERT INTO @QuestSeed VALUES
...
USING @QuestSeed AS source
ON target.scenario_code = source.scenario_code AND target.quest_code = source.quest_code
WHEN MATCHED THEN UPDATE SET
    ...
    is_active = 1
WHEN NOT MATCHED THEN INSERT
    (scenario_code, quest_code, quest_title, quest_goal, quest_step_json, sort_order, is_active)
    VALUES
    (source.scenario_code, source.quest_code, source.quest_title, source.quest_goal, source.quest_step_json, source.sort_order, 1);

UPDATE target
SET is_active = 0
FROM dbo.DaGo_Authoring_Scenario_Quest AS target
WHERE target.scenario_code = N'xiaocheng_jiushi'
  AND NOT EXISTS (
      SELECT 1 FROM @QuestSeed AS source
      WHERE source.scenario_code = target.scenario_code
        AND source.quest_code = target.quest_code
  );
GO

SELECT
    N'xiaocheng_jiushi' AS scenario_code,
    N'1.12.0-modular-runtime' AS da_go_version,
    ...
    (SELECT COUNT(*) FROM dbo.DaGo_Authoring_Scenario_NPC WHERE scenario_code = N'xiaocheng_jiushi' AND is_active = 1) AS active_npc_count,
    (SELECT COUNT(*) FROM dbo.DaGo_Authoring_Scenario_Quest WHERE scenario_code = N'xiaocheng_jiushi' AND is_active = 1) AS active_quest_count;
GO
